...
REG_CANONICAL = {
    ...
    "对应适应症专项指导原则": None,
    # 同一法规的不同写法归一
    "医疗器械生产质量管理规范附录·独立软件（2022）": "医疗器械生产质量管理规范附录·独立软件",
    # 联网核查修正（2026-06）：
    # - YY/T 0316-2016 未废止，与 GB/T 42062-2022 现行并行，不再合并，各自独立保留
    # - 国家层面无"创新医疗器械特别审查程序（2023修订）"，现行为 2018年第83号
    # - 真实世界数据指导原则正确文号为 2020年第77号（非2021）
    "创新医疗器械特别审查程序（2023修订）": "创新医疗器械特别审查程序（2018年第83号）",
    "医疗器械真实世界数据用于临床评价技术指导原则（试行）（2021）": "真实世界数据用于医疗器械临床评价技术指导原则（试行）（2020第77号）",
    # 补抽用了带说明的全称，checklist 用短编号，归一到带说明全称（信息更全）
    "YY/T 0664-2020": "YY/T 0664-2020（医疗器械软件 软件生存周期过程，对应IEC 62304）",
    "GB/T 42062-2022": "GB/T 42062-2022（医疗器械 风险管理对医疗器械的应用，对应ISO 14971）",
}


def canonical_regs(name: str) -> list[str]:
    """把一个 basis 里的法规名规范化为 0~N 个干净法规名（拆合写、并变体、剔占位）。
    注意：只按【空格+斜杠+空格】拆合写（如 "GB/T 42062 / YY/T 0316"），
    不能拆 GB/T、YY/T 标准号里的斜杠。"""
    out = []
    for part in re.split(r"\s+/\s+", name or ""):   # 拆 "A / B" 合写（斜杠两侧必须有空格）
        part = part.strip()
        if not part:
            continue
